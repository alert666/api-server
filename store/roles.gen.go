...
func newRole(db *gorm.DB, opts ...gen.DOOption) role {
	...
	_role.Users = roleManyToManyUsers{
		...
		Oauth2User: struct {
			...
			RelationField: field.NewRelation("Users.Oauth2User", "model.Oauth2User"),
			User: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Users.Oauth2User.User", "model.User"),
			},
		},
		Roles: struct {
			field.RelationField
			...
			RelationField: field.NewRelation("Users.Roles", "model.Role"),
			Users: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Users.Roles.Users", "model.User"),
			},
			Apis: struct {
				field.RelationField
				Roles struct {
				...
				RelationField: field.NewRelation("Users.Roles.Apis", "model.Api"),
				Roles: struct {
					field.RelationField
				}{
					RelationField: field.NewRelation("Users.Roles.Apis.Roles", "model.Role"),
				},
			},
		},
	}

	_role.Apis = roleManyToManyApis{
		db: db.Session(&gorm.Session{}),
...
func (r role) clone(db *gorm.DB) role {
	...
	r.Apis.db = db.Session(&gorm.Session{Initialized: true})
	r.Apis.db.Statement.ConnPool = db.Statement.ConnPool
	return r
}
...
func (r role) replaceDB(db *gorm.DB) role {
	...
	r.Apis.db = db.Session(&gorm.Session{})
	return r
}
...
type roleManyToManyApis struct {
	db *gorm.DB

	field.RelationField
}

func (a roleManyToManyApis) Where(conds ...field.Expr) *roleManyToManyApis {
	if len(conds) == 0 {
		return &a
	}
...
func (a roleManyToManyApis) WithContext(ctx context.Context) *roleManyToManyApis {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a roleManyToManyApis) Session(session *gorm.Session) *roleManyToManyApis {
	a.db = a.db.Session(session)
	return &a
}

func (a roleManyToManyApis) Model(m *model.Role) *roleManyToManyApisTx {
	return &roleManyToManyApisTx{a.db.Model(m).Association(a.Name())}
}

func (a roleManyToManyApis) Unscoped() *roleManyToManyApis {
	a.db = a.db.Unscoped()
	return &a
}

type roleManyToManyApisTx struct{ tx *gorm.Association }

func (a roleManyToManyApisTx) Find() (result []*model.Api, err error) {
	return result, a.tx.Find(&result)
}

func (a roleManyToManyApisTx) Append(values ...*model.Api) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleManyToManyApisTx) Replace(values ...*model.Api) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleManyToManyApisTx) Delete(values ...*model.Api) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleManyToManyApisTx) Clear() error {
	return a.tx.Clear()
}

func (a roleManyToManyApisTx) Count() int64 {
	return a.tx.Count()
}

func (a roleManyToManyApisTx) Unscoped() *roleManyToManyApisTx {
	a.tx = a.tx.Unscoped()
	return &a
}
